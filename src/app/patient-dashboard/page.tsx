...
export default function PatientDashboardPage() {
  ...
  useEffect(() => {
    ...
    // Check this user is actually a patient
    supabase
      .from("profiles")
      .select("role, name")
      .eq("id", user.id)
  ...
  async function addMedication(e: React.FormEvent) {
    ...
    const { data, error } = await supabase
      .from("medications")
      .insert({ user_id: user.id, name: newMed.trim() })
      .select()
      .single();
  ...
  async function toggleTaken(id: string, currentValue: boolean) {
    ...
    await supabase
      ...
      .eq("id", id);
    setMedications((prev) =>
      prev.map((m) => (m.id === id ? { ...m, taken: !currentValue } : m))
    );
  }
    ...
    <div className="min-h-screen bg-gray-bg-light">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <SignOutButton redirectUrl="/">
                <button
                  type="button"
                  className="ml-1 p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-colors cursor-pointer"
                  title="Sign out"
                >
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
        ...
        {totalCount > 0 && (
          <div className="bg-white rounded-2xl border border-gray-100 p-6 mb-6 shadow-sm">
            ...
            <div className="w-full h-3 bg-gray-100 rounded-full overflow-hidden">
              <div
                className="h-full bg-linear-to-r from-primary to-blue-400 rounded-full transition-all duration-500"
                style={{
        ...
        <form
          ...
          className="bg-white rounded-2xl border border-gray-100 p-4 mb-6 shadow-sm flex gap-3"
        >
          ...
          >
            {adding ? (
              ...
              <>
                ...
                <span className="hidden sm:inline">{l.addBtn}</span>
              </>
            )}
          </button>
        </form>
              ...
              {medications.map((med) => (
                <div
                  ...
                  className={`px-6 py-4 flex items-center gap-4 transition-colors ${
                    med.taken ? "bg-green-50/30" : "hover:bg-gray-50/50"
                  }`}
                >
                  <button
